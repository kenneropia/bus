...
// const run = async () => {
//   const seedTable = async <T>(count: number, cb: (i: number) => Promise<T>) => {
//     const resultArray: T[] = [];
//     let i = 1;
//     while (i < count) {
//       const res = await cb(i);
...
//   const seedProduct = await seedTable(productData.length, async (i) => {
//     return await db.product.create({
//       data: {
//         name: productData[i].name,
//         description: productData[i].description,
//         price: productData[i].price,
//         quantityLeft: productData[i].quantityLeft,
